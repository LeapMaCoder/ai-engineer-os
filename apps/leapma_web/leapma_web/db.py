...
_MYSQL_DDL = [
  ...
  username VARCHAR(64) NULL UNIQUE COMMENT '登录名（唯一；NULL=未注册/游客体系）',
  email VARCHAR(255) NULL COMMENT '邮箱（可空）',
  password_hash VARCHAR(255) NULL COMMENT '密码哈希（可空）',
  display_name VARCHAR(64) NULL COMMENT '显示昵称（可空）',
...
# (name, sqlite_type, mysql_type_with_comment) — MySQL ADD COLUMN 须含中文 COMMENT（D-058）
_AUTH_COLUMNS = (
    (
        "username",
        "TEXT UNIQUE",
        "VARCHAR(64) NULL UNIQUE COMMENT '登录名（唯一；NULL=未注册/游客体系）'",
    ),
    ("email", "TEXT", "VARCHAR(255) NULL COMMENT '邮箱（可空）'"),
    ("password_hash", "TEXT", "VARCHAR(255) NULL COMMENT '密码哈希（可空）'"),
    ("display_name", "TEXT", "VARCHAR(64) NULL COMMENT '显示昵称（可空）'"),
    (
        "concept_mode_default",
        "TEXT",
        "VARCHAR(16) NULL COMMENT '默认概念模式（normal|story；NULL=normal）'",
    ),
)
